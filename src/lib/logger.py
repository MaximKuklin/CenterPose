...
class Logger(object):
    ...
    def img_summary(self, tag, value_list, step):
        if USE_TENSORBOARD:
            if value_list is not None:
                for prefix, value in enumerate(value_list):
                    for i, v in value.items():
                        im_rgb = cv2.cvtColor(v, cv2.COLOR_BGR2RGB)
                        self.writer.add_image(f"{tag}_{i}_{prefix}",
                                              im_rgb,
                                              step,
                                              dataformats="HWC",
                                              )
